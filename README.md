...
- **Auth context** – `AuthProviderWithGoogle` wraps the app and exposes `signIn`, `signOut`, `user`, and `isLoading`
...
- **`app/_layout.tsx`** – Root layout. It wraps the entire app in `AuthProviderWithGoogle` and renders the Expo Router `Stack`. All screens have access to the auth context (`user`, `signIn`, `signOut`, `isLoading`).
...
3. **Package name**: must match `app.json` → `expo.android.package` → `com.khushali.expoauth`.
...
3. **Bundle ID**: must match `app.json` → `expo.ios.bundleIdentifier` → `com.khushali.expoauth`.
...
- [ ] Android OAuth client created (package `com.khushali.expoauth`, SHA-1) → Client ID as `EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID`
- [ ] iOS OAuth client created (bundle ID `com.khushali.expoauth`) → Client ID as `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`
...
- **iOS `bundleIdentifier`**: `com.khushali.expoauth` – must match the iOS OAuth client in Google Cloud.
- **Android `package`**: `com.khushali.expoauth` – must match the Android OAuth client in Google Cloud.
